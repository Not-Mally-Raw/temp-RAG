"""
Manufacturing RAG Database System
Specialized vector database for manufacturing design rules and guidelines
"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for k, v in asdict(self).items():
            ...
            if isinstance(v, list):
                result[k] = ', '.join(str(item) for item in v) if v else None
            else:
                result[k] = v
...
class ManufacturingRAG:
    ...
    def process_pdf_document(self, pdf_bytes: bytes, filename: str) -> Dict[str, int]:
        ...
        results = {"text_chunks": 0, "table_chunks": 0, "image_chunks": 0, "warnings": []}
        
        # Process text content
        try:
            print(f"\n🔍 Starting text extraction for: {filename}")
            sentences = extract_sentences(pdf_bytes)
            print(f"📄 Extracted {len(sentences) if sentences else 0} sentences")
            ...
            if sentences:
                ...
                print(f"📦 Created {len(text_documents)} chunks")
                
                self._add_documents_to_vectorstore(text_documents)
                results["text_chunks"] = len(text_documents)
                print(f"✅ Successfully added {len(text_documents)} text chunks to RAG database")
            else:
                warning_msg = f"⚠️ No text extracted from {filename}. This PDF may be: scanned images, password-protected, or have complex layout."
                print(warning_msg)
                results["warnings"].append(warning_msg)
        
        except Exception as e:
            error_msg = f"Error processing text from {filename}: {e}"
            print(f"❌ {error_msg}")
    ...
    def _add_documents_to_vectorstore(self, documents: List[Document]):
        """Add documents to the vector store with sanitized metadata."""
        if documents:
            texts = [doc.page_content for doc in documents]
            # Double-sanitize metadata to ensure ChromaDB compatibility
            # (Document.metadata might retain original types)
            metadatas = [self._sanitize_metadata(doc.metadata) for doc in documents]
            
            # Debug: Check first metadata
            if metadatas:
                print(f"🔍 Sample metadata types: {[(k, type(v).__name__) for k, v in list(metadatas[0].items())[:5]]}")
            
            # Add texts to vector store
            self.vectorstore.add_texts(texts, metadatas)
            
            # Note: Modern Chroma persists automatically, no manual persist() needed
    ...
    def _sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in metadata.items():
            if value is None:
                ...
                sanitized[key] = ', '.join(str(v) for v in value) if value else None
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        # Calculate total chunks (only count numeric values, skip warnings)
        total_chunks = 0
        for doc in self.doc_registry.values():
            chunks_data = doc.get("chunks", {})
            for key, value in chunks_data.items():
                # Only sum numeric chunk counts, skip 'warnings' and other non-numeric fields
...
def integrate_with_text_pipeline(pdf_bytes: bytes, filename: str, rag_system: ManufacturingRAG):
